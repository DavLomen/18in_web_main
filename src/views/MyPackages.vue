  ...
  <div class="HomePage" id="app">
    ...
    <div class="app-wrapper" style="width: calc(100% - 25rem);">
      ...
      <div class="container">
        <div class="t-table">
          <div  class="tr">
            <div  class="th">{{$t('package[0]')}}</div>
            <div  class="th">{{$t('package[1]')}}</div>
            <div  class="th">{{$t('package[2]')}}</div>
            <div  class="th">{{$t('product[4]')}}</div>
            <div  class="th">{{$t('product[2]')}}</div>
            <div  class="th">{{$t('package[3]')}}</div>
            <div  class="th">{{$t('product[14]')}}</div>
            <div  class="th">{{$t('next')}}</div>
          </div>
          <div class="no-data" v-if="listData.length==0">{{$t('vanPull[1]')}}</div>
          <div class="tr" v-for="(item,index) in listData" :key="index">
            <div class="td">{{item.order_number}}<br>{{item.addtime_txt}}</div>
            <div class="td">{{item.title}}</div>
            <div class="td">{{InitData.currency}} {{item.day_income}}</div>
            <div class="td">{{InitData.currency}} {{item.total_income}}</div>
            <div class="td">{{item.cycle}} {{$t('product[15]')}}</div>
  ...
  export default {
    ...
    methods: {
      jumpShow(item) {
        ...
        this.$router.push('/contractDetail?view=1&id='+item.pid)
      },
      changePage(page){
        this.pageNo = page;
