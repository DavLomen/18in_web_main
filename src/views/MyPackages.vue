  ...
  <div class="HomePage" id="app">
    ...
    <div class="d-flex">
      ...
      <div class="container col-lg-10 ">
        <div class="">
          <h4 class="text-zs">{{$t('transaction[6]')}}</h4>
          <div class="table-responsive--md">
            <table class="table custom--table">
              <thead class="fs-5 border-bottom border-hhs bg-okh" style="height: 50px">
              <tr class="">
                <th   class="text-center">{{$t('package[1]')}}</th>
                <th   class="text-center">{{$t('package[11]')}}</th>
                <th   class="text-center">{{$t('package[10]')}}</th>
                <th   class="text-center">{{$t('package[2]')}}</th>
                <th   class="text-center">{{$t('package[12]')}}</th>
                <!--                    <th   class="text-center">{{$t('product[14]')}}</th>-->
                <th   class="text-center">{{$t('package[8]')}}</th>
                <th   class="text-center">{{$t('package[9]')}}</th>
              </tr>
              </thead>
              ...
              <tr v-for="(item,index) in listData" :key="index">
                ...
                <!--                    </td>-->
                <td :data-label="$t('package[1]')" class="text-center small">
                  {{item.title}}
                </td>

                <td :data-label="$t('package[11]')" class="text-center small">
                  {{item.cycle}} {{$t('product[15]')}}
                </td>
                <td :data-label="$t('package[10]')" class="text-center small">
                  {{item.amount}}
                </td>
                <td :data-label="$t('package[2]')" class="text-center small">
                  {{InitData.currency}} {{item.day_income}}
                </td>
                <td :data-label="$t('package[12]')" class="text-center small">
                  {{InitData.currency}} {{item.total_income}}
                </td>
                <!--                    <td  class="text-center small">-->
                <!--                      <div class="td" @click="jumpShow(item)">{{$t('dashboard[0]')}}</div>-->
                <!--                    </td>-->
                <td :data-label="$t('package[8]')" class="text-center small">
                  {{item.addtime_txt}}
                </td>
                <td :data-label="$t('package[9]')" class="text-center small">
                  {{item.end_time}}
                </td>
                <!--                    <td  class="text-center small">-->
