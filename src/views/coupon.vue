  ...
  <div class="HomePage" id="app">
    ...
    <div class="app-wrapper" style="width: calc(100% - 25rem);">
      <div class="page-header">
        <div class="page-title">{{$t('coupon[0]')}}</div>
      ...
      <div class="container">
        <div style="width: 50%;">
          <van-tabs @change="changeTabs">
            <van-tab :title="$t('coupon[6]')" name="1">

            </van-tab>
            <van-tab :title="$t('coupon[7]')" name="2">

            </van-tab>
            <van-tab :title="$t('coupon[8]')" name="3">
            ...
            <div class="table-responsive--md">
              <table class="table custom--table" style="width: 100%">
                ...
                <tr v-for="(item,index) in listData" :key="index">
                  <td :data-label="$t('coupon[9]')">
                    {{item.coupon_name}}
                  </td>
                  <td :data-label="$t('coupon[10]')" >
                    <div v-html="item.description"></div>
                  </td>
                  <td :data-label="$t('coupon[11]')">
                    {{item.discount}}%
                  </td>
                  <td :data-label="$t('coupon[12]')">
                    {{item.expiretime}}
                  </td>
                  <td :data-label="$t('coupon[13]')">
                    <span v-if="item.status==1">{{$t('coupon[6]')}}</span>
                    <span v-if="item.status==2">{{$t('coupon[7]')}}</span>
                    <span v-if="item.status==3">{{$t('coupon[8]')}}</span>
                  </td>
                </tr>
                </tbody>
              </table>
...
<!--        <div class="t-table">-->
<!--          <div class="tr">-->
<!--            <div class="th">{{$t('coupon[9]')}}</div>-->
<!--            <div class="th">{{$t('coupon[10]')}}</div>-->
<!--            <div class="th">{{$t('coupon[11]')}}</div>-->
<!--            <div class="th">{{$t('coupon[12]')}}</div>-->
<!--            <div class="th">{{$t('coupon[13]')}}</div>-->
<!--          </div>-->
<!--          <div class="no-data" v-if="listData.length==0">{{$t('vanPull[1]')}}</div>-->
<!--          <div class="tr" v-for="(item,index) in listData" :key="index">-->
<!--            <div class="td">{{item.coupon_name}}</div>-->
<!--            <div class="td" v-html="item.description"></div>-->
<!--            <div class="td">{{item.discount}}%</div>-->
<!--            <div class="td">{{item.expiretime}}</div>-->
<!--            <div class="td">-->
<!--              <span v-if="item.status==1">{{$t('coupon[6]')}}</span>-->
<!--              <span v-if="item.status==2">{{$t('coupon[7]')}}</span>-->
<!--              <span v-if="item.status==3">{{$t('coupon[8]')}}</span>-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        msg_title:"",
        msg_content:"",
        status:1,//status=1 未使用 status=2  已使用 status=3  已过期
      }
    },
    ...
    methods: {
      ...
      getListData(type) {
        ...
        this.$Model.MYCOUPON({page_no:this.pageNo,status:this.status},data=>{
          this.isLoad = false
          if(data){
