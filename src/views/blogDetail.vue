    ...
    <div class="app-wrapper">
      <section class="sp_pt_120 sp_pb_120">
        <div class="container blogdetail">
          <div class="row gy-4">
            <div class="col-lg-8">
                <div style="width: 100%;text-align: right;padding: 10px;font-size: 1.5rem">
                  <button @click="$router.go(-1)" style="background: linear-gradient(to right, #00f785, #410093);
    color: #fff;
    border-radius: 5px;">{{$t('recharge.default[7]')}}</button>
                </div>
              <div class="blog-details-img">
                <img v-if="showInfo.cover_img !=''" :src="showInfo.cover_img" height="400px" width="100%" alt="blog">
              </div>
              <div class="blog-details-content mt-4">
                <h3 class="title mb-3">{{showInfo.title}}</h3>
                <p class="text-justifys"></p>
                <div v-html="showInfo.content" class="content"></div>
              </div>
            </div>
            <div class="col-lg-4 ps-lg-4">
              <div class="blog-widget">
                <h4 class="blog-widget-title">{{$t('blog[1]')}}</h4>
                <div class="short-post-wrapper">
                  <div @click="$router.push('/blogDetail/'+item.id)" class="short-post" v-for="(item,index) in listData" :key="index">
                    <div class="thumb">
                      <img :src="item.cover_img" alt="image">
                    </div>
                    <div class="content">
                      <h5 class="title">
                        <a href="javascript:;">
  ...
  export default {
    ...
    watch: {
      '$route'(to, from) {
        ...
        this.$router.go(0);
        let artid = this.articleId;
        let that = this;
        this.$Model.noticeDetail({id:artid}, data => {
    ...
    created() {
      ...
      let artid = this.articleId;
      _this.$Model.noticeDetail({id:artid}, data => {
