  ...
  <div class="HomePage" id="app">
    ...
    <div class="d-flex">
      ...
      <div class="container col-lg-10 ">
        <div class="">
          <div class="p-4 border rounded-3 border-0" style="background: linear-gradient(to left, #1989fa, #f8f9fa);">
            <h4>{{$t('affli[0]')}}</h4>
            <h6>{{$t('affli[1]')}}</h6>
            <div class="mt-4">
              <a href="javascript:;" @click="$router.push('/affiliate')" type="button" class="btn col-6 col-lg-6 p-2 text-light" style="background-color: #0d6efdb5;">{{$t('affli[2]')}}</a>
            </div>
          </div>
          <div class="py-3">
            <div class="m-2 pb-2">
              <h4 class="text-zs1">{{$t('affli[3]')}}</h4>
            </div>
            <div class="border rounded-3 border-0 bg-okh p-3 text-center">
              <input v-model="promoteUrl" class="w-100 border rounded-3 border-0 ps-3 my-3" style="line-height: 20px; height: 40px; font-size: 15px; color: #0d6efdb5; background-color: #fff;" type="text"  disabled="" data-sharkid="__0">
              <button @click="copy" :data-clipboard-text="promoteUrl" id="copylink" class="btn btn-primary col-4 p-2 copy">{{$t('affli[4]')}}</button>
            </div>
            <div class="border rounded-3 border-0 bg-okh p-3 text-center mt-3">
              <h4 class="text-zs1">{{$t('affli[5]')}}</h4>
              <h2>{{teamNum}}</h2>
            </div>
          </div>
          <div class=" ">
            <h4 class="text-zs1">{{$t('affli[6]')}}</h4>

            <table class="w-100 text-bhs text-center ">
              <thead class="fs-5 border-bottom border-hhs bg-okh" style="height: 50px;">
              <tr class="">
                <th class="col-3">{{$t('affli[7]')}}</th>
                <th class="col-3">{{$t('affli[8]')}}</th>
                <th class="col-3">{{$t('affli[9]')}}</th>
                <th class="col-3">{{$t('affli[10]')}}</th>
              </tr>
              </thead>
