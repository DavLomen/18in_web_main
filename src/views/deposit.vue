      ...
      <div v-else class="container col-lg-10 ">
        <div class="col-lg-12">
          <div class="border rounded-4 border-0 bg-okh p-3 single-settings-box top-flashlight light-xl leftside" style="position: relative; overflow: visible;">
            ...
            <input class="w-100 border rounded-3 border-0 ps-3 my-2" id="amount" style="line-height: 28px; height: 60px; font-size: 15px; color: rgb(163,163,163); background-color: rgb(51, 51, 51);" type="number" v-model="postData.money" name="amount" data-sharkid="__0">
          ...
          <div>
            ...
            <table class="w-100 text-bhs text-center mt-3 " id="statusTable">
              ...
              <tr class="border-bottom border-hhs">
                ...
                <th  scope="col-3" class="text-center">{{$t('deposit[14]')}}</th>
                <th  scope="col-2" class="text-center">{{$t('deposit[9]')}}</th>
                <th  scope="col-2" class="text-center">{{$t('deposit[16]')}}</th>
                <th  scope="col-3" class="text-center">{{$t('deposit[17]')}}</th>
              </tr>
              </thead>
              ...
              <tr v-for="(item,index) in listData" :key="index">
                ...
                <td  class="text-center">
                  <div class="badge fw-medium text-bg-primary" v-if="item.status==3">{{item.status_desc}}</div>
                  <div class="badge fw-medium text-bg-warning" v-else-if="item.status==-1">{{item.status_desc}}</div>
                  <div class="" v-else>{{item.status_desc}}</div>
                </td>
