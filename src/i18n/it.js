export default {
  "line": "Linea",
  "language": "Lingua",
  "common": [
    "Servizio",
    "Annullare la sospensione",
    "Sorteggio fortunato"
  ],
  "vanPull": [
    "Niente più dati",
    "Nessun dato"
  ],
  "login": {
    "text": [
      "Ricorda USEMAME/password",
      "Registrazione ...",
      "Accedi ora"
    ],
    "placeholder": [
      "Si prega di inserire il nome utente",
      "Inserisci la password di accesso"
    ],
    "i18n": [
      "Non hai un account?",
      "Registro"
    ]
  },
  "register": {
    "text": [
      "registro",
      "Codice SMS che invia ...",
      "Ottieni il codice SMS",
      "Registrazione ...",
      "Registrati ora",
      "Avere un account, scarica l'app",
      "Accetta i termini del disclaimer"
    ],
    "placeholder": [
      "Inserisci il tuo nome utente",
      "Inserisci il codice di verifica SMS",
      "Immettere la password di accesso",
      "Conferma la tua password",
      "Inserisci il codice di invito",
      "Due password sono diverse",
      "Inserisci il codice di verifica",
      "Si prega di seguire per accettare i termini del disclaimer",
      "Si prega di inserire e -mail"
    ],
    "i18n": [
      "Hai un account? ",
      "Login"
    ]
  },
  "user": {
    "default": [
      "Il mio superiore",
      "Account",
      "Codice di invito",
      "ESCI AGGIORNAMENTO",
      "Bilancia",
      "Oro",
      "Il mio portafoglio"
    ]
  },
  "userInfo": {
    "default": [
      "Informazioni personali",
      "Ritratto di testa",
      "Numero di cellulare",
      "Carta di banca",
      "Alipay",
      "Informazioni dettagliate",
      "Password di accesso",
      "Password di pagamento",
      "Fare clic su Impostazioni",
      "Cambia il ritratto della testa",
      "Modificare la password di accesso",
      "Modificare la password di pagamento",
      "Invia",
      "Svuotare la cache"
    ],
    "label": [
      "Password di accesso originale",
      "Nuova password di accesso",
      "Conferma della password",
      "Password di pagamento originale",
      "Nuova password di pagamento",
      "Conferma la password di pagamento"
    ],
    "placeholder": [
      "Immettere la password di accesso originale",
      "Immettere una nuova password di accesso",
      "Si prega di confermare la password di accesso",
      "Immettere la password di pagamento originale",
      "Inserisci la nuova password di pagamento",
      "Si prega di confermare la password di pagamento"
    ]
  },
  "userSet": {
    "default": [
      "Certificazione del nome reale",
      "Informazioni dettagliate",
      "Lega Alipay",
      "Invia"
    ],
    "label": [
      "Vero nome"
    ],
    "placeholder": [
      "Si prega di inserire un vero nome"
    ]
  },
  "bankCard": {
    "default": [
      "Lega la carta di banca",
      "Autentica ora",
      "Invio ...",
      "Aggiungilo ora",
      "Aggiungi carta di banca"
    ],
    "tips": [
      "Aggiungi la tua carta di banco dopo l'autenticazione del nome reale",
      "Il nome di apertura del conto della tua carta bancaria vincola deve essere uguale al tuo vero nome verificato, altrimenti non sarai in grado di prelevare correttamente."
    ],
    "label": [
      "Nome",
      "Nome bancario",
      "Conto bancario",
      "Codice IFSC"
    ],
    "placeholder": [
      "Inserisci il nome della banca",
      "Si prega di inserire il conto bancario",
      "Inserisci il codice IFSC",
      "Inserisci il nome"
    ]
  },
  "wallet": {
    "default": [
      "Il mio portafoglio",
      "Ricaricare",
      "Ritiro",
      "Record di ricarica",
      "Record di prelievo"
    ],
    "label": [
      "Metodo di ritiro",
      "Importo di prelievo",
      "Password di pagamento",
      "Invia",
      "Numero di telefono",
      "E-mail",
      "Ifsc",
      "Banca di prelievo",
      "Commissione di gestione"
    ],
    "pix_type": [
      "Tipo di chiave pix",
      "PIX",
      "CPF",
      "Tipo di account",
      "Branch Bank",
      "Codice bancario",
      "Numero di ramo",
      "Conto bancario"
    ],
    "pix_type_placeholder": [
      "Seleziona il tipo di pix",
      "Inserisci la foto giusta",
      "Inserisci il CPF",
      "Seleziona il tipo di account",
      "Inserisci la banca delle filiali",
      "Inserisci il "
    ],
    "placeholder": [
      "Seleziona Metodo di prelievo",
      "Si prega di inserire l'importo del prelievo",
      "Si prega di inserire la password di pagamento",
      "Seleziona il metodo di prelievo",
      "Inserisci il numero di cellulare del beneficiario",
      "Inserisci l'indirizzo e -mail del beneficiario",
      "Inserisci il beneficiario IFSC",
      "Seleziona la banca di prelievo"
    ],
    "msg": [
      "Non hai impostato la password di pagamento, impostare prima",
      "Non hai vinto il tuo indirizzo di prelievo, ti preghiamo di vincola prima"
    ]
  },
  "recharge": {
    "default": [
      "Ricaricare",
      "I dettagli di ricarica",
      "Record di ricarica",
      "Bilancia disponibile {denaro}, scegli il metodo di ricarica",
      "L'importo minimo singolo è {valuta} {min}, massimo {valuta} {max}",
      "Invio ...",
      "Ricarica ora",
      "Ritorno",
      "Caricamento..."
    ],
    "label": [
      "Importo di ricarica",
      "Canale di ricarica",
      "Nome del trasferimento",
      "Il nome del pagatore",
      "Pagatore del telefono cellulare",
      "Conto di pagatore UPI",
      "Email del pagatore"
    ],
    "placeholder": [
      "Inserisci l'importo della ricarica",
      "Seleziona il canale di ricarica",
      "Inserisci il nome del trasferimento",
      "L'importo minimo singolo è {valuta} {min}",
      "L'importo massimo singolo è {valuta} {max}",
      "Inserisci il nome del pagatore",
      "Inserisci il numero di cellulare del pagatore, prefissa il numero con il codice di area internazionale, come +86",
      "Inserisci il conto UPI del pagatore",
      "Inserisci l'indirizzo email del pagatore"
    ],
    "info": [
      "Importo di ricarica",
      "Numero d'ordine",
      "Banca",
      "Account n",
      "Nome",
      "Copia",
      "{type} trasferimento",
      "Schermata di trasferimento",
      "Invia",
      "Codice IFSC",
      "Tipo di account",
      "Istruzioni di elaborazione"
    ],
    "tips": [
      "Selezionare il seguente metodo per trasferire l'importo della corrispondenza per evitare ritardi nel controllo finanziario.<br>Dopo il trasferimento, caricare lo screenshot di trasferimento come certificato di verifica",
      "Non c'è bisogno di aggiungere amici, scansionare il codice QR per trasferirmi denaro",
      "Completa il trasferimento in base alle informazioni di seguito",
      "Suggerimento: dopo il pagamento riuscito, contattare il servizio online per fornire il numero di conto del tuo membro, importo di ricarica, numero di ordine, numero di conto depositante, tempo di ricarica, comodo per il dipartimento finanziario per aggiungere fondi per te in tempo.",
      "NOTA: si prega di inviare un pagamento per ogni trasferimento",
      "Dopo il trasferimento, caricare lo screenshot di trasferimento come certificato di verifica",
      "Selezionare il seguente metodo per trasferire l'importo corrispondente",
      "Carica uno screenshot del trasferimento"
    ]
  },
  "fundRecord": {
    "default": [
      "Record di spesa",
      "Record di ricarica",
      "Registri delle entrate"
    ],
    "tabs": [
      "Reddito",
      "Spesa",
      "Ricaricare"
    ]
  },
  "help": [
    "Helpbook",
    "Non disponibile",
    "Domande frequenti",
    "Se non sei riuscito a trovare una risposta alle tue domande, contatta il nostro supporto clienti",
    "1. Cos'è il cloud mining?",
    "Il cloud Mining è un metodo di estrazione di criptovaluta in cui gli individui in affitta la potenza di calcolo da data center remoti, eliminando la necessità di acquistare e mantenere l'hardware. ",
    "2. Come posso iniziare con {WT}?",
    "Registrati per un account di abbonamento gratuito utilizzando il tuo indirizzo email, quindi completa il tuo account, scegli i contratti di mining in cui desideri investire e inizia a guadagnare rendimenti giornalieri!",
    "3. Cosa sono i vantaggi {WT}?",
    "Unisciti a {WT}, in cui non è necessario hardware/software/competenza tecnica, né è necessario ricercare le tendenze del mining cloud o le fluttuazioni del mercato. ",
    "4. Posso acquistare più di un contratto di mining di cloud?",
    "Sì, puoi acquistare più contratti, anche contratti di termini diversi.",
    "5. Quando vengono pagati i premi?",
    "I premi vengono pagati quotidianamente e prelevabili in qualsiasi momento.",
    "6. Devo pagare i costi di elettricità e manutenzione?",
    "No, nessuna commissione extra. ",
    "7. Esiste un importo di acquisto minimo o massimo?",
    "Gli importi minimi di acquisto differiscono per ogni contratto, è possibile trovarlo nella nostra pagina dei prezzi.",
    "8. Esiste un importo di prelievo minimo?",
    "Il nostro deposito e prelievo minimo sono $ 50.",
    "9. Quali metodi di pagamento supporti?",
    "Puoi pagare per i contratti di mining utilizzando criptovalute come BTC, ETH, LTC, USDT.",
    "10. Qual è il programma di affiliati?",
    "Ogni cliente ha un link affiliato unico, che può essere condiviso con chiunque, copia il tuo link affiliati e condividi questo link per ottenere bonus. "
  ],
  "dialog": [
    "Suggerimenti gentili",
    "Confermare",
    "Invio ...",
    "Copia è riuscita",
    "La versione del sistema iOS è bassa e non supportata",
    "Registrazione ...",
    "Caricamento dei dati ..."
  ],
  "messageInfo": [
    "Hai nuove notizie, fai clic qui per visualizzare",
    "Visualizza il messaggio",
    "Avviso",
    "Data",
    "Titolo",
    "Dettaglio"
  ],
  "lucky": [
    "Numero rimanente di estrazioni:",
    "Sfortunatamente, non hai vinto il premio! ",
    "Record utente vincente",
    "Record della lotteria",
    "Ottieni premi",
    "Regole della lotteria",
    "Invita un livello di stella 1, i subordinati attivano il livello delle stelle per la prima volta e diventa un dipendente dell'azienda e può partecipare alla lotteria una volta",
    "Congratulazioni per aver vinto il premio!  ",
    "Scusa, non sei idoneo per la lotteria!",
    "Giradischi di fortuna"
  ],
  "usdt": [
    "Valuta",
    "Catena",
    "Indirizzo del portafoglio",
    "Aggiungi indirizzo del portafoglio",
    "Indirizzo di trasferimento",
    "Numero di transazione (TXID)",
    "Indirizzo del portafoglio"
  ],
  "usdtp": [
    "Inserisci l'indirizzo del portafoglio",
    "Seleziona Tipo di valuta",
    "Immettere l'ID transazione (TXID)"
  ],
  "foot": [
    "DRML Miner è la piattaforma di cloud mining leader al mondo. Abbiamo un team di professionisti e una tecnologia avanzata. Dopo anni di sviluppo e miglioramento, l'azienda utilizza energia pulita come energia cinetica per rendere il cloud mining più semplice ed efficiente. La sua missione è chiara: fornirti gli strumenti e i servizi necessari per avere successo nel campo delle risorse digitali e creare più ricchezza per gli utenti. Unisciti a noi e porta con te un viaggio redditizio nella criptovaluta.",
    "Link principali",
    "Collegamenti importanti",
    "Chi siamo",
    "FAQ",
    "Affiliati",
    "Blog",
    "Contatto",
    "Collegamenti importanti",
    "Privacy e politica",
    "Termini di servizio",
    "Copyright © 2018-2025 DRML MINER。 All Rights",
    "IMPARARE",
    "Cos'è la criptovaluta?",
    "Cos'è Web3?",
    "Risorse",
    "AZIENDA",
    "Centro di aiuto"
  ],
  "head": [
    "Casa",
    "App",
    "Iscrizione",
    "Registrazione",
    "Pannello di controllo",
    "Logout",
    "Menu",
    "Depositare",
    "Ritirare",
    "Contratti",
    "Contratti minerari",
    "I miei contratti",
    "Transazioni",
    "Messaggio",
    "Impostazioni",
    "Indirizzo del portafoglio",
    "Password di accesso",
    "Password di pagamento"
  ],
  "about": [
    "Certificato di incorporazione",
    "Numero aziendale",
    "Indirizzo della sede legale",
    "Tipo di azienda",
    "Incorporato",
    "Controlla online",
    "Scarica PDF",
    "Una piattaforma di mining di cloud di fiducia",
    "Posizioni dei data center",
    "Sistemi di alimentazione",
    "Sistema di raffreddamento",
    "Sicurezza/protezione antincendio",
    "Hardware del data center",
    "Il nostro team",
    "Ciclo di energia eolica",
    "Ciclo di energia solare",
    "Team di esperti",
    "Abbiamo un eccellente team di ingegneri blockchain ed esperti IT per offrirti i servizi tecnici più professionali.",
    "Profitti stabili",
    "I tuoi guadagni minerari vengono automaticamente sistemati sul tuo account piattaforma ogni 24 ore.",
    "Supporto dal vivo 24/7",
    "Contatta il nostro assistenza clienti con le tue domande in qualsiasi momento."
  ],
  "walletAddress": [
    "Indirizzo del portafoglio",
    "Password di accesso",
    "Password di pagamento",
    "Criptovaluta",
    "Inserisci il tuo indirizzo del portafoglio",
    "Invia"
  ],
  "transaction": [
    "Txid",
    "Transatto",
    "Quantità",
    "Post Balance",
    "Tipo di transazione"
  ],
  "regLogin": [
    "Hai già un account?",
    "Nome utente",
    "Si prega di inserire il nome utente",
    "Indirizzo e-mail",
    "Inserisci il tuo nome utente o e -mail",
    "Password",
    "Password almeno 6 caratteri",
    "Conferma password",
    "Codice affiliato (opzionale)",
    "Verificare il codice",
    "Accetto",
    "ISCRIZIONE",
    "Non hai un account?",
    "Iscriviti ora",
    "Il tuo codice di verifica non può essere vuoto.",
    "Iscrivendosi, accetti il ​​nostro",
    "politica sulla riservatezza",
    "E",
    "Termini",
    "CREARE UN ACCOUNT",
    "Hai già un account?",
    "REGISTRAZIONE",
    "Accedi per continuare",
    "La tua email non può essere vuota."
  ],
  "settle": [
    "Interesse di transazione",
    "Ogni 24 ore"
  ],
  "product": [
    "Prezzo del contratto",
    "Immettere l'importo",
    "Periodo di investizione",
    "Utile netto giornaliero",
    "Utile netto totale",
    "Reddito fisso",
    "Risolvere gli interessi",
    "Ogni 24 ore",
    "Potere hash",
    "Minatore",
    "Bonus di affiliazione",
    "Contratto di locazione hash",
    "Inizia ora",
    "Esaurito",
    "Dettagli del contratto",
    "Giorni",
    "Hash Power Marketplace",
    "Restituire dopo la scadenza",
    "Giorno",
    "Contratti minerari",
    "Offriamo una varietà di contratti di investimento minerario del cloud, garantendo profitti stabili e affidabili.",
    "Premi fissi da giorno:",
    "Volume",
    "(Esaurito al 100%)",
    "Ordina ora",
    "Vedere di più",
    "Guardare",
    "Quantità",
    "Password",
    "Quantità",
    "Paga"
  ],
  "password": [
    "password attuale",
    "Nuova password",
    "Conferma password",
    "Conferma le modifiche"
  ],
  "package": [
    "Txid | tempo",
    "Contrarre",
    "Ritorno",
    "Quantità",
    "Guadagni minerari",
    "Data",
    "Dall'investimento",
    "Guadagni minerari"
  ],
  "affliliate": [
    "Programma di affiliati",
    "ISCRIZIONE",
    "Unisciti alla nostra comunità, iscrivi un account, ottieni il tuo link affiliati.",
    "Invitare gli utenti",
    "Invita gli utenti, condividendo il link delle affiliate tramite i social media.",
    "Guadagnare commissioni",
    "Dopo che un nuovo utente registra e si iscrive al piano contrattuale, riceverai una ricompensa del 3% dell'importo del piano contrattuale.",
    "Unisciti al nostro programma di affiliazione (programma partner) e guadagna una commissione a vita fino al 3%! ",
    "Ad esempio, se qualcuno registra un account utilizzando il tuo codice di affiliazione e investe in un piano da $ 100, riceverai $ 3 gratuitamente. ",
    "Quindi, perché non cogliere l'opportunità? ",
    "Unisciti a noi"
  ],
  "affliliate2": [
    "Sull'affiliato",
    "Fare riferimento a un amico e iniziare a guadagnare con il nostro programma di riferimento"
  ],
  "affliliate1": [
    "Il nostro programma di affiliazione offre due principali vantaggi",
    "1. Offriamo un programma di affiliazione di alto livello che ti offre molti vantaggi. ",
    "2. Quando il numero di utenti validi che consigli nel mese raggiunge il numero richiesto, è possibile richiedere un bonus di stipendio di promozione mensile!",
    "① Raccomanda 8 nuovi utenti di registrarsi e investire per l'aggiornamento a VIP1 e oltre e puoi ottenere $ 350 al mese",
    "② Raccomanda 25 nuovi utenti di registrarsi e investire per l'aggiornamento a VIP1 e oltre e puoi ottenere $ 1.300 al mese",
    "③ Consiglia 50 nuovi utenti per registrarsi e investire per l'aggiornamento a VIP1 e oltre e puoi ottenere $ 4.000 al mese",
    "④ Raccomanda 100 nuovi utenti di registrarsi e investire per l'aggiornamento a VIP1 e oltre e puoi ottenere $ 11.000 al mese",
    "⑤ Raccomanda 200 nuovi utenti di registrarsi e investire per l'aggiornamento a VIP1 e oltre e puoi ottenere $ 22.000 al mese",
    "Nota: contattare il servizio clienti l'ultimo giorno di ogni mese per richiedere lo stipendio degli affiliati. ",
    "Unisciti a noi",
    "Unisciti al nostro programma partner e guadagna fino al 3%+1,5% commissioni a vita! ",
    "Per esempio:",
    "A raccomanda B di effettuare ulteriori investimenti; ",
    ""
  ],
  "clickmore": [
    "Fai clic di più"
  ],
  "oursay": [
    "Cosa dicono i nostri rispettabili clienti"
  ],
  "say": [
    "DRML Miner è un'eccellente piattaforma di mining di cloud che può essere utilizzata per estrarre le criptovalute per guadagnare entrate extra. ",
    "Lavoro su questa piattaforma da quasi un anno. ",
    "DRML Miner è un'ottima piattaforma di mining di cloud. ",
    "Ho lavorato su DRML Miner per sei anni. ",
    "Sono molto grato a DRML Miner per avermi aiutato quando ho avuto solo $ 10 in tasca tre anni fa. ",
    "La sicurezza della piattaforma DRML Miner è molto buona. ",
    "Grazie a Dio per avermi permesso di incontrare DRML Miner Questa piattaforma non ha commissioni nascoste per i prelievi. ",
    "Sono una madre single. ",
    "Ho visto l'articolo che hanno condiviso nelle notizie. ",
    "Ho trovato la piattaforma DRML Miner sulla pagina di raccomandazione di Google. ",
    "Mi piace molto l'affidabilità complessiva della piattaforma e il supporto clienti e le spese giornaliere della piattaforma sono stabili ed è molto facile guadagnare reddito. ",
    "Sono felice di condividere di nuovo il mio feedback su DRML Miner. ",
    "L'effetto complessivo della piattaforma DRML Miner è molto buono. ",
    "Questa è la prima volta che contatta il cloud mining come un modo per fare soldi. ",
    "DRML Miner tratta molto bene i nuovi arrivati. ",
    "Il mio amico mi ha presentato DDB Miner, ho ricevuto un bonus in contanti immediatamente dopo essermi registrato. ",
    "Uso DRML Miner per un po '. ",
    "Ho provato troppe altre piattaforme di mining di cloud. ",
    "Consiglierò DRML Miner a tutti gli appassionati di criptovaluta. ",
    "DRML Miner rende il mining di bitcoin più facile che mai. ",
    ""
  ],
  "affiliates": [
    "Affiliati",
    "Programma di affiliazione",
    "La tua email",
    "Il tuo nome utente",
    "Il tuo codice di riferimento",
    "Il tuo link di affiliazione",
    "Guadagnare azioni bonus",
    "Condividi ad altri. Otterrai il 3% in più come bonus del loro acquisto su ogni ordine. ",
    "Saperne di più",
    "Elenco dei partner",
    "I tuoi partner",
    "Data",
    "Da",
    "Livello",
    "Quantità",
    "Tipo",
    "Copia link"
  ],
  "appDown": [
    "Inizia il viaggio minerario",
    "Gestisci le tue attività {WT} in viaggio utilizzando le nostre app mobili. ",
    "Gestisci le tue entrate in qualsiasi momento, ovunque",
    "Easy inizio con il mining",
    "Abbiamo semplificato tutti i passaggi che dovrai iniziare con il cloud mining sull'app.",
    "Estrazione con un clic",
    "Ordinare senza sforzo piani di mining a breve e breve termine con soglie basse e tasse basse.",
    "Gestione in tempo reale",
    "Sia in loco o via, monitora e gestisci senza sforzo le risorse dei piani di mining.",
    "Supporto di chat 24/7",
    "Contatta il nostro assistenza clienti con le tue domande in qualsiasi momento."
  ],
  "contact": [
    "Mettiti in contatto",
    "Nome",
    "E-mail",
    "Soggetto",
    "Messaggio",
    "Entra in captcha",
    "Invia ora",
    "Chattare con il supporto",
    "Parla con la nostra squadra amichevole",
    "Chattare con online",
    "Supporto dal vivo 24/7",
    "Contatta il nostro assistenza clienti con le tue domande in qualsiasi momento.",
    "Visitaci",
    "Visita il nostro quartier generale dell'ufficio"
  ],
  "dashboard": [
    "Visualizza tutto",
    "Equilibrio disponibile",
    "Commissione totale",
    "Ordini attivi",
    "Ordini scaduti",
    "Ordini totali",
    "Depositi totali",
    "Prelievi totali",
    "Il tuo nome utente",
    "Il tuo codice di riferimento",
    "Affiliate Link",
    "Copia link",
    "Partner",
    "Ottieni supporto ora",
    "Il mio collegamento di riferimento",
    "Fondi di prova",
    "Profitti totali",
    "Sistema"
  ],
  "faq": [
    "Domande frequenti"
  ],
  "home": [
    "Benvenuti in {WT}",
    "● L'accesso giornaliero guadagna $ 0,5",
    "I nuovi utenti possono ottenere $ 10 quando si iscrivono",
    "Conosci",
    "Come lavoriamo?",
    "Segui questi semplici passaggi e fai profitto!",
    "Registrazione istantanea",
    "Clic",
    "Iscrizione",
    "Per riempire lo spazio vuoto, il nostro 256 SSL proteggerà la tua privacy.",
    "Fare un investimento",
    "Login",
    "Il tuo account per fare clic su Invest per iniziare a guadagnare il profitto.",
    "ottenere profitti",
    "Otterrai il tuo profitto sul tuo profilo, otterrai anche un pagamento immediato",
    "Grande visione. ",
    "Verde",
    "Energia",
    "Utilizziamo pannelli solari di alto livello per l'alimentazione, garantendo i profitti proteggendo l'ambiente.",
    "Assicurato dal design",
    "La sicurezza dei dati e dei nostri server è stata una priorità per noi da D-1. ",
    "Contratti redditizi",
    "Ordina facilmente piani di estrazione a lungo e breve termine con basse barriere e tasse basse.",
    "Mercato stabile",
    "I tuoi guadagni minerari vengono automaticamente sistemati sul tuo account piattaforma ogni 24 ore.",
    "Sicurezza del fondo al 100%",
    "La maggior parte dei fondi sono conservati in modo sicuro in portafogli freddi offline. ",
    "Supporto amichevole",
    "Il nostro team di supporto è disponibile 24 ore su 24, 7 giorni su 7, per assicurarsi che tutto stia andando bene. ",
    "",
    "",
    "",
    "",
    "",
    ""
  ],
  "withdraw": [
    "Equilibrio disponibile",
    "Rete di prelievo",
    "Quantità",
    "Password di pagamento",
    "Ricevibile",
    "Invia",
    "Txid",
    "Quantità",
    "Stato di trading",
    "Tempo",
    "Ritirare",
    "Ritirare i record",
    "Ritiro l'indirizzo"
  ],
  "deposit": [
    "Depositare",
    "Criptovaluta",
    "Rete di deposito",
    "Registri di deposito",
    "Numero ID transazione / hash",
    "Compila l'ID transazione / hash",
    "Carica screenshot di trasferimento",
    "Invia",
    "Equilibrio disponibile",
    "Metodo di deposito",
    "Quantità",
    "Limite",
    "Pagabile",
    "Invia",
    "Txid",
    "Metodo di deposito",
    "Quantità",
    "Stato",
    "Tempo",
    "Dettagli",
    "Ricompensa",
    "Dopo la ricompensa",
    "Tasso di conversione",
    "Importo pagabile",
    "Importo effettivo",
    "Carica screenshot di trasferimento",
    "Prossimo",
    "Si prega di trasferire al seguente indirizzo",
    "Il sistema confermerà automaticamente la ricevuta sulla catena del blocco di memoria e la pubblicherà sul saldo del tuo account.",
    "Importo del deposito minimo:",
    ", quantità inferiore a",
    "non verrà accreditato sul tuo account.",
    "Ogni trasferimento in verrà registrato sulla catena del blocco di memoria e non può essere manomesso",
    "Indirizzo di trasferimento",
    "Convertire",
    "Carica screenshot",
    "Si prega di trasferire l'importo esatto per l'indirizzo"
  ],
  "limits": [
    "Ogni account è limitato a un acquisto al giorno.",
    "Ogni utente può acquistare solo una volta"
  ],
  "productDetail": [
    "Dettaglio",
    "Servizi di tasso di hash e post-vendita forniti da DRML Miner.",
    "I piani di tasso di hash cloud forniscono una soluzione di mining facile ed efficiente per i clienti istituzionali. \n",
    "Introduzione al prodotto\n",
    "1. Istruzioni di acquisto\n",
    "Prima di effettuare un ordine, si prega di capire attentamente il prodotto. \n",
    "2. Insediamento di output\n",
    "Il reddito stimato deriva dal funzionamento in tempo reale della rete di attività digitali ed è soggetto a modifiche dinamiche. \n",
    "3. Output di calcolo\n",
    "Per il calcolo delle entrate minerarie, consultare il pool di minerali Poolin: https://www.poolin.com\n",
    "4. Terminazione del contratto\n",
    "Il contratto verrà interrotto automaticamente quando scade. Il contratto di mining diventa attivo immediatamente confrmazione del pagamento. A meno altrimenti fornito dalla legge apportabile o da una particolare offerta, tutti gli acquisti sono definitivi e non rimborsabili. Grazie per la tua comprensione.\n",
    "5. Suggerimenti per il rischio\n",
    "Il prezzo delle attività digitali spesso fluttua notevolmente e la difficoltà mineraria di ciascuna valuta verrà regolata regolarmente. \n",
    "La piattaforma si riserva il diritto di interpretazione finale per i termini del presente contratto.\n",
    "Disclaimer e supporto\n",
    "1. Come specificato nelle condizioni d'uso di DRML Miner, una volta che un ordine è stato pagato, è immutabile e non rimborsabile.\n",
    "2.DRML Miner non sarà responsabile per eventuali risultati che derivano da rischi incontrollabili, inclusi ma non limitati a catastrofi naturali come inondazioni, eruzioni vulcaniche, terremoti, frane, fuochi, tempeste, altri eventi meteorologici a rischio, azioni del governo, azioni del governo, azioni meteorologiche, ecc.\n",
    "3. La fluttuazione di hashrate è inevitabile. \n",
    "4. L'output statico è stato calcolato staticamente dalla difficoltà attuale, che fluttua con il mercato. ",
    ""
  ],
  "blog": [
    "Blog recente",
    "Post di blog recenti",
    "Per saperne di più"
  ],
  "welfare": [
    "Club VIP"
  ],
  "tips": [
    "Dopo il pagamento corretto, salva lo screenshot di pagamento sul telefono cellulare e carichi lo screenshot nella pagina prima di inviare l'ordine. ",
    "Lo screenshot deve contenere l'hash della transazione, altrimenti il ​​sistema non può riconoscerlo."
  ],
  "next": "Prossimo paga",
  "placehoder": [
    "Inserisci l'importo",
    "Inserisci la password di pagamento",
    "Si consiglia la password di accesso e la password di pagamento per essere diverse, ricorda la password."
  ],
  "dep": [
    "RICORDO CALDO: l'importo minimo di ricarica inizia a $ 100. ",
    "Suggerimenti: l'importo minimo di prelievo parte da 100 USD. Se non si sa come prelevare o non hai la password, è possibile contattare il servizio clienti online per assistenza."
  ],
  "deposit1": [
    "Hai richiesto",
    "Per favore paga",
    "per pagamenti con successo",
    "Dopo il successo del pagamento, salva lo screenshot e carica lo screenshot come richiesto."
  ],
  "header": [
    "CASA",
    "PANNELLO DI CONTROLLO",
    "Le mie bollette",
    "DEPOSITARE",
    "RITIRARE",
    "I miei ordini",
    "Contratti",
    "Programma di affiliazione",
    "IMPOSTAZIONI",
    "CHI SIAMO",
    "Centro di aiuto",
    "Impostazioni",
    "Le mie bollette",
    "REGISTRAZIONE",
    "ISCRIZIONE",
    "DISCONNESSIONE"
  ],
  "home1": [
    "Easy Cloud Mining in qualsiasi momento, ovunque",
    "",
    "",
    "DRML Miner è la piattaforma di mining di cloud leader al mondo, che utilizza energia pulita combinata con apparecchiature di alto livello per fornire agli utenti un modo semplice e conveniente per estrarre. ",
    "Inizia il tuo investimento di minerario del cloud, preleva il tuo primo pagamento dopo 24 ore.",
    "Inizia a estrarre",
    "Come iniziare",
    "ISCRIZIONE",
    "Compila il modulo e preparati per iniziare il mining entro 1 minuto.",
    "Scegli i tuoi contratti",
    "Ti forniamo contratti di mining di cloud redditizi e privi di rischi.",
    "Inizia a guadagnare",
    "Dopo aver acquistato il contratto, i profitti giornalieri verranno prontamente depositati sul tuo account.",
    "Scegli i contratti di mining",
    "Offriamo una varietà di contratti di investimento minerario del cloud, garantendo profitti stabili e affidabili.",
    "Perché scegliere noi?",
    "Profitti stabili",
    "Offriamo un reddito fisso costante che viene automaticamente depositato sul tuo account ogni giorno.",
    "Pagamenti regolari rapidi",
    "Tutte le richieste di prelievo verranno elaborate automaticamente dal sistema entro cinque minuti.",
    "Inclusività",
    "Il nostro team di investimento esperto e esperti IT ti forniranno la migliore tecnologia leader del mercato.",
    "Team di esperti",
    "La nostra piattaforma offre supporto per gli investimenti minerari di cloud a tutti, anche se sei un principiante, puoi iniziare rapidamente e beneficiare da esso.",
    "I nostri partner",
    "Accetta una varietà di metodi di pagamento",
    "Per garantire comodità per i nostri investitori, {WT} fornisce una vasta gamma di metodi di pagamento sia per il deposito di fondi che per la ricezione di pagamenti.",
    "Investimenti per il mining di cloud in Texas",
    "<p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Benvenuti in {WT}, il tuo biglietto per l'eccitante mondo del mining di criptovaluta. Che tu sia un principiante o un investitore esperto, la nostra piattaforma offre una serie di opportunità per esplorare e trarre profitto dal regno della valuta digitale.</p><p class=\"mt-2 s-content\" ><span class=\"bullet-icon\" >•</span>Nel cuore del Texas, inizia il viaggio di WT}, sfruttando le vaste risorse e l'abilità tecnologica dello stato per fornire soluzioni di mining di cloud all'avanguardia.</p><p class=\"mt-2 s-content\" ><span class=\"bullet-icon\" >•</span>In {WT}, non ci stiamo solo per estrarre l'innovazione. </p><p class=\"mt-2 s-content\" ><span class=\"bullet-icon\" >•</span>Il nostro hardware di cloud mining a Washington vanta attrezzature di prim'ordine, garantendo prestazioni e affidabilità ottimali. </p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Dì addio a configurazioni complesse; con il bitcoin di cloud mining in California, è possibile accedere ai nostri servizi online. Il nostro servizio di cloud Mining Bitcoin è progettato pensando alla facilità d'uso, per i principianti e gli esperti.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Prendiamo sul serio la tua sicurezza, offrendo servizi di mining di cloud affidabili sostenuti da robuste misure. {WT} è noto per la sua trasparenza e affidabilità, rendendoci una scelta preferita per il mining di cloud, non solo in Arizona, ma negli Stati Uniti.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>La nostra dedizione alla tecnologia blockchain è evidente nel nostro minerario cloud blockchain negli Stati Uniti. Utilizziamo blockchain pubbliche per esperienze di mining migliorate e la nostra maggiore sicurezza mineraria del cloud garantisce la sicurezza delle tue risorse.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Da Litecoin a Ethereum, ti abbiamo coperto. </p><p class=\"mt-2 s-content\" >Ma non finisce qui. {WT} ti offre un'opportunità unica di trarre profitto dalla tua passione. Gioca i nostri programmi di affiliazione di cloud mining e il programma di affiliazione cripto negli Stati Uniti, trasformando il tuo interesse per la criptovaluta in una fonte di reddito.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Con {WT}, il tuo viaggio nell'universo di criptovaluta inizia. Avvia il mining, inizia a guadagnare e inizia a garantire il tuo futuro finanziario oggi.</p>",
    "29"
  ],
  "latest": [
    "Ultime transazioni",
    "Ultima ricarica",
    "Ultimi prelievi",
    "Utente",
    "Quantità",
    "Tempo"
  ],
  "reg": [
    "Inserisci la tua email"
  ],
  "tutorial": [
    "Tutorial"
  ],
  "bounty": [
    "Generosità"
  ],
  "invest": [
    "Investire ora"
  ],
  "ppp": [
    "Questo contratto non partecipa ai premi dell'alleanza",
    "(Questo contratto può essere illimitato o più copie possono essere acquistate contemporaneamente)"
  ],
  "buy": [
    "Acquista ora",
    "Inserisci fondi di acquisto"
  ],
  "aa": [
    "Si prega di copiare l'indirizzo del portafoglio della piattaforma e trasferire i fondi tramite il ritiro di scambio \"il tuo\". "
  ],
  "tag": [
    "Si prega di compilare il tag XRP:"
  ],
  "about1": [
    "CHI SIAMO",
    "DRML Miner è la piattaforma di mining di cloud leader al mondo, che utilizza energia pulita combinata con apparecchiature di alto livello per fornire agli utenti un modo semplice e conveniente per estrarre. ",
    "DRML Miner è stata fondata nel 2018 ed ha sede nel Regno Unito. ",
    "Corso aziendale: miglioramento continuo e innovazione",
    "Dopo anni di sviluppo e crescita continua, attualmente ha più di 100 miniere di energia ecologiche su larga scala in tutto il mondo, con utenti in 180 paesi e regioni. ",
    "Missione: guidare una nuova era di energia verde",
    "Ci impegniamo a diventare un leader innovativo nel campo del cloud computing e cambiare il modo in cui gli umani usano energia. ",
    "Visione: costruire un marchio leader globale nel settore del cloud computing",
    "Il nostro obiettivo è quello di diventare un leader globale nell'innovativa tecnologia di mining di cloud verdi, fornire prodotti e servizi a valore aggiunto agli utenti e creare un valore di investimento eccezionale e rendimenti elevati. ",
    "Autorizza i tuoi guadagni con DRML Miner",
    "Sperimenta il mining digitale senza soluzione di continuità, sicuro e redditizio con la tecnologia e i servizi all'avanguardia di DRML Miner.",
    "Il mio più intelligente, guadagnare più velocemente - DRML Miner lo rende facile!",
    "Nessun hardware, nessun problema, solo premi di Bitcoin puri.",
    "Massimizza i profitti con le soluzioni di mining intelligenti di DRML Miner.",
    "Sicuro, scalabile e senza soluzione di continuità: il futuro dell'estrazione delle criptovalute.",
    "Il tuo percorso minerario inizia qui: unisciti a DRML Miner oggi!"
  ],
  "download1": [
    "Scarica l'app mobile per iniziare il mining oggi!",
    "Fai clic per scaricare direttamente l'app mobile DRML Miner per gestire le tue attività di mining sempre e ovunque, goditi un'esperienza più veloce e ti aiuta ad aumentare il tuo reddito in modo efficiente."
  ],
  "newhome": [
    "Sfruttare l'energia verde per guidare un futuro sostenibile per il mining di nuvole",
    "DRML Miner Data Center utilizza energia verde pulita, a basso costo (principalmente idroelettrica, energia eolica, energia solare, ecc.), Pratiche di concetti di protezione ambientale, promuove l'innovazione a risparmio energetico e si impegna a costruire un data center verde, sostenibile e risparmio energetico.",
    "Registra un account",
    "Fare clic sull'opzione Pagina Web (iscrivi) per compilare il modulo",
    "Seleziona il contratto",
    "Scegli il tuo contratto minerario",
    "Ottenere profitti",
    "Il reddito contrattuale stabile viene automaticamente aggiornato sul tuo account",
    "Perché scegliere noi?",
    "Vantaggi del minatore DRML",
    "Ultimo hardware",
    "Utilizziamo le ultime macchine minerarie ASIC e attrezzature GPU e utilizziamo energia pulita combinata con apparecchiature di alto livello per fornire agli utenti un modo semplice e conveniente per estrarre.",
    "Nessuna commissione aggiuntiva",
    "Non è necessario hardware, forniamo la potenza di elaborazione e la piattaforma si occupa del processo di mining, non è necessario pagare per le commissioni di installazione o ulteriori costi di manutenzione e elettricità. ",
    "Guadagni stabili",
    "Una volta acquistato il contratto, il nostro sistema inizia automaticamente a lavorare ed elabora i pagamenti ogni 24 ore.",
    "Sicurezza dei fondi",
    "La maggior parte dei fondi sono conservati in modo sicuro in portafogli freddi offline. ",
    "Team di esperti",
    "Il nostro team minerario è composto da professionisti del settore blockchain e ingegneri IT, in piedi 24 ore su 24, 7 giorni su 7, per garantire che tutto vada senza intoppi e siamo sempre qui per aiutarti.",
    "Mining ecologico",
    "Le nostre macchine minerarie e i sistemi di raffreddamento sono alimentati dai migliori pannelli solari monocristallini e utilizzano anche l'energia eolica su larga scala per garantire i profitti proteggendo meglio l'ambiente.",
    "Cosa otterrai come partner affiliato?",
    "Unisciti al nostro programma di riferimento (programma partner) e guadagna fino all'1,5% -4,5% Commissione per la vita! ",
    "Innanzitutto, iscriviti per un account e ottieni il tuo collegamento/URL di riferimento.",
    "Successivamente, invita i nuovi utenti condividendo il collegamento di referral tramite i social media o il tuo sito Web. ",
    "Quando un nuovo utente registra il collegamento di riferimento, riceverai un bonus dell'1,5% -4,5% dell'importo dell'acquisto. ",
    ""
  ],
  "complete": "completato",
  "cancel": "Cancellare",
  "confirm": "Confermare"
}
