export default {
  "line": "Linha",
  "language": "Linguagem",
  "common": [
    "Serviço",
    "Cancelar a suspensão",
    "Lucky Draw"
  ],
  "vanPull": [
    "Não há mais dados",
    "Sem dados"
  ],
  "login": {
    "text": [
      "Lembre -se de uso de uso/senha",
      "Registro ...",
      "Faça login agora"
    ],
    "placeholder": [
      "Por favor, insira o nome de usuário",
      "Por favor, insira a senha de login"
    ],
    "i18n": [
      "Não tem uma conta?",
      "Registrar"
    ]
  },
  "register": {
    "text": [
      "registrar",
      "Código SMS enviando ...",
      "Obtenha código SMS",
      "Registrando ...",
      "Registre -se agora",
      "Tenha uma conta, download de aplicativo",
      "Concorde com os termos do aviso"
    ],
    "placeholder": [
      "Por favor, insira seu nome de usuário",
      "Por favor, insira o código de verificação do SMS",
      "Por favor, insira a senha de login",
      "Por favor, confirme sua senha",
      "Por favor, insira o código de convite",
      "Duas senhas são diferentes",
      "Por favor, insira o código de verificação",
      "Marque para concordar com os termos do aviso",
      "Insira o email"
    ],
    "i18n": [
      "Tem uma conta? ",
      "Conecte-se"
    ]
  },
  "user": {
    "default": [
      "Meu superior",
      "Conta",
      "Código de convite",
      "EXIT LOGIN",
      "Equilíbrio",
      "Ouro",
      "Minha carteira"
    ]
  },
  "userInfo": {
    "default": [
      "Informações pessoais",
      "Retrato da cabeça",
      "Número de telemóvel",
      "Cartão bancário",
      "Alipay",
      "Informações detalhadas",
      "Senha de login",
      "Senha de pagamento",
      "Clique em Configurações",
      "Mude o retrato da cabeça",
      "Altere a senha de login",
      "Alterar senha de pagamento",
      "Enviar",
      "Esvazie o cache"
    ],
    "label": [
      "Senha de login original",
      "Nova senha de login",
      "Confirmando a senha",
      "Senha de pagamento original",
      "Nova senha de pagamento",
      "Confirme a senha de pagamento"
    ],
    "placeholder": [
      "Por favor, insira a senha de login original",
      "Por favor, insira uma nova senha de login",
      "Confirme a senha de login",
      "Digite a senha de pagamento original",
      "Por favor, insira a nova senha de pagamento",
      "Confirme a senha de pagamento"
    ]
  },
  "userSet": {
    "default": [
      "Certificação de nome real",
      "Informações detalhadas",
      "Bind Alipay",
      "Enviar"
    ],
    "label": [
      "Nome real"
    ],
    "placeholder": [
      "Por favor, insira um nome real"
    ]
  },
  "bankCard": {
    "default": [
      "Vincular um cartão bancário",
      "Autentique agora",
      "Enviando ...",
      "Adicione agora",
      "Adicione o cartão bancário"
    ],
    "tips": [
      "Adicione seu cartão bancário após autenticação de nome real",
      "O nome de abertura da conta do seu cartão bancário encadernado deve ser o mesmo que seu nome real verificado, caso contrário, você não poderá retirar com sucesso."
    ],
    "label": [
      "Nome",
      "Nome do banco",
      "Conta bancária",
      "Código IFSC"
    ],
    "placeholder": [
      "Por favor, insira o nome do banco",
      "Por favor, insira a conta bancária",
      "Por favor, insira o código IFSC",
      "Por favor, insira o nome"
    ]
  },
  "wallet": {
    "default": [
      "Minha carteira",
      "Recarregar",
      "Cancelamento",
      "Recarregar o registro",
      "Registro de retirada"
    ],
    "label": [
      "Método de retirada",
      "Valor de retirada",
      "Senha de pagamento",
      "Enviar",
      "Número de telefone",
      "E-mail",
      "Ifsc",
      "Banco de retirada",
      "Taxa de manuseio"
    ],
    "pix_type": [
      "Tipo de chave pix",
      "Pix",
      "CPF",
      "Tipo de conta",
      "Banco da agência",
      "Código bancário",
      "Número da filial",
      "Conta bancária"
    ],
    "pix_type_placeholder": [
      "Selecione Pix Type",
      "Por favor, insira a pix certa",
      "Por favor, insira o CPF",
      "Selecione o tipo de conta",
      "Por favor, digite o banco da filial",
      "Por favor, digite o "
    ],
    "placeholder": [
      "Selecione o método de retirada",
      "Insira o valor da retirada",
      "Insira a senha de pagamento",
      "Selecione o método de retirada",
      "Por favor, insira o número do celular do beneficiário",
      "Por favor, insira o endereço de e -mail do beneficiário",
      "Por favor, insira o beneficiário sesc",
      "Selecione o banco de retirada"
    ],
    "msg": [
      "Você não definiu senha de pagamento, defina -a primeiro",
      "Você não vinculou seu endereço de retirada, vincule -o primeiro"
    ]
  },
  "recharge": {
    "default": [
      "Recarregar",
      "Recarregue os detalhes",
      "Recarregar o registro",
      "Saldo disponível {Money}, escolha o método de recarga",
      "O valor mínimo único é {moeda} {min}, máximo {moeda} {max}",
      "Enviando ...",
      "Recarregue agora",
      "Retornar",
      "Carregando..."
    ],
    "label": [
      "Quantidade de recarga",
      "Recarregar o canal",
      "Nome da transferência",
      "Nome do pagador",
      "Telefone celular do pagador",
      "Conta do pagador upi",
      "Email do pagador"
    ],
    "placeholder": [
      "Por favor, insira o valor da recarga",
      "Selecione o canal de recarga",
      "Por favor, insira o nome da transferência",
      "O valor mínimo único é {moeda} {min}",
      "O valor máximo único é {moeda} {max}",
      "Por favor, insira o nome do pagador",
      "Por favor, insira o número do celular do pagador, prefixo o número com o código da área internacional, como +86",
      "Por favor, insira a conta da UPI do pagador",
      "Por favor, insira o endereço de e -mail do pagador"
    ],
    "info": [
      "Quantidade de recarga",
      "Número do pedido",
      "Banco",
      "Conta não",
      "Nome",
      "Cópia",
      "{type} transfer",
      "Transferir captura de tela",
      "Enviar",
      "Código IFSC",
      "Tipo de conta",
      "Instruções de processamento"
    ],
    "tips": [
      "Selecione o seguinte método para transferir o valor correspondente para evitar atraso na verificação financeira.<br>Após a transferência, envie a captura de tela de transferência como um certificado de verificação",
      "Não há necessidade de adicionar amigos, digitalize o código QR para transferir dinheiro para mim",
      "Preencha a transferência de acordo com as informações abaixo",
      "Dica: Após o pagamento bem -sucedido, entre em contato com o Serviço Online para fornecer o número da sua conta de membro, recarregar o valor, número do pedido, número da conta do depositante, tempo de recarga, conveniente para o departamento financeiro adicionar fundos para você a tempo.",
      "Nota: Envie um pagamento para cada transferência",
      "Após a transferência, envie a captura de tela de transferência como um certificado de verificação",
      "Selecione o seguinte método para transferir o valor correspondente",
      "Faça o upload de uma captura de tela da transferência"
    ]
  },
  "fundRecord": {
    "default": [
      "Registro de despesas",
      "Recarregar o registro",
      "Registros de receita"
    ],
    "tabs": [
      "Receita",
      "Gasto",
      "Recarregar"
    ]
  },
  "help": [
    "Ajuda",
    "Não disponível",
    "Perguntas frequentes",
    "Se você não conseguir encontrar uma resposta para suas perguntas, entre em contato com o suporte ao cliente",
    "1. O que é mineração em nuvem?",
    "A mineração em nuvem é um método de mineração de criptomoedas, onde os indivíduos arrendam o poder de computação de data centers remotos, eliminando a necessidade de comprar e manter o hardware. ",
    "2. Como faço para começar com {WT}?",
    "Registre -se para uma conta de associação gratuita usando seu endereço de e -mail e, em seguida, complete sua conta, escolha os contratos de mineração em que deseja investir e comece a obter retornos diários!",
    "3. O que são vantagens {WT}?",
    "Junte -se a {WT}, onde você não precisa de hardware/software/experiência técnica, nem precisa pesquisar tendências de mineração em nuvem ou flutuações de mercado. ",
    "4. Posso comprar mais de um contrato de mineração em nuvem?",
    "Sim, você pode comprar vários contratos, mesmo contratos de diferentes termos.",
    "5. Quando as recompensas são pagas?",
    "As recompensas são pagas diariamente e retiradas a qualquer momento.",
    "6. Preciso pagar custos de eletricidade e manutenção?",
    "Não, sem taxas extras. ",
    "7. Existe um valor mínimo ou máximo de compra?",
    "Os valores mínimos de compra diferem para cada contrato, você pode encontrá -lo em nossa página de preços.",
    "8. Existe algum valor mínimo de retirada?",
    "Nosso depósito e retirada mínimos são de US $ 50.",
    "9. Quais métodos de pagamento você apoia?",
    "Você pode pagar por contratos de mineração usando criptomoedas como BTC, ETH, LTC, USDT.",
    "10. Qual é o programa de afiliados?",
    "Todo cliente tem um link de afiliados exclusivos, que pode ser compartilhado com qualquer pessoa, copie seu link de afiliados e compartilhe esse link para obter bônus. "
  ],
  "dialog": [
    "Dicas gentis",
    "Confirmar",
    "Enviando ...",
    "A cópia foi bem -sucedida",
    "A versão do sistema iOS é baixa e não suportada",
    "Registrando ...",
    "Carregamento de dados ..."
  ],
  "messageInfo": [
    "Você tem notícias novas, clique aqui para ver",
    "Exibir mensagem",
    "Perceber",
    "Data",
    "Título",
    "Detalhe"
  ],
  "lucky": [
    "Número restante de sorteios:",
    "Infelizmente, você não ganhou o prêmio! ",
    "Registro de usuário vencedor",
    "Registro de sorteio",
    "Obtenha prêmios",
    "Regras de loteria",
    "Convide um nível de estrela 1, os subordinados ativam o nível de estrela pela primeira vez e se tornam um funcionário da empresa e podem participar da loteria uma vez",
    "Parabéns por ganhar o prêmio!  ",
    "Desculpe, você não é elegível para a loteria!",
    "Placa giratória de sorte"
  ],
  "usdt": [
    "Moeda",
    "Corrente",
    "Endereço da carteira",
    "Adicione o endereço da carteira",
    "Endereço de transferência",
    "Número da transação (TXID)",
    "Endereço da carteira"
  ],
  "usdtp": [
    "Por favor, insira o endereço da carteira",
    "Selecione o tipo de moeda",
    "Por favor, insira o ID da transação (TXID)"
  ],
  "foot": [
    "O DRML Miner é a plataforma de mineração em nuvem líder mundial. Temos uma equipe profissional e tecnologia avançada. Após anos de desenvolvimento e melhoria, a empresa usa energia limpa como energia cinética para tornar a mineração em nuvem mais simples e eficiente. Sua missão é clara: fornecer a você as ferramentas e serviços necessários para ter sucesso no campo de ativos digitais e criar mais riqueza para os usuários. Junte-se a nós e traga a você uma jornada lucrativa de criptomoeda.",
    "Links principais",
    "Links importantes",
    "Sobre nós",
    "Perguntas frequentes",
    "Afiliados",
    "Blog",
    "Contato",
    "Links importantes",
    "Privacidade e política",
    "Termos de Serviços",
    "Copyright © 2018-2025 DRML MINER。 TODOS os direitos",
    "APRENDER",
    "O que é criptomoeda?",
    "O que é web3?",
    "RECURSOS",
    "EMPRESA",
    "Centro de ajuda"
  ],
  "head": [
    "Lar",
    "App",
    "Inscrever-se",
    "Entrar",
    "Painel",
    "Logout",
    "Menu",
    "Depósito",
    "Retirar",
    "Contratos",
    "Contratos de mineração",
    "Meus contratos",
    "Transações",
    "Mensagem",
    "Configurações",
    "Endereço da carteira",
    "Senha de login",
    "Senha de pagamento"
  ],
  "about": [
    "Certificado de incorporação",
    "Número da empresa",
    "Endereço do escritório registrado",
    "Tipo de empresa",
    "Incorporado em",
    "Verifique online",
    "Baixe PDF",
    "Uma plataforma de mineração de nuvem confiável",
    "Locais de data center",
    "Sistemas de energia",
    "Sistema de resfriamento",
    "Segurança/proteção contra incêndio",
    "Hardware do data center",
    "Nossa equipe",
    "Ciclo de energia eólica",
    "Ciclo de energia solar",
    "Equipe especialista",
    "Temos uma excelente equipe de engenheiros de blockchain e especialistas em TI para fornecer os serviços técnicos mais profissionais.",
    "Lucros estáveis",
    "Seus ganhos de mineração são resolvidos automaticamente na sua conta de plataforma a cada 24 horas.",
    "Suporte ao vivo 24/7",
    "Entre em contato com o suporte ao cliente com suas perguntas a qualquer momento."
  ],
  "walletAddress": [
    "Endereço da carteira",
    "Senha de login",
    "Senha de pagamento",
    "Criptomoeda",
    "Por favor, insira seu endereço de carteira",
    "Enviar"
  ],
  "transaction": [
    "Txid",
    "Trocado",
    "Quantia",
    "Pós -equilíbrio",
    "Tipo de transação"
  ],
  "regLogin": [
    "Já tem uma conta?",
    "Nome de usuário",
    "Insira o nome de usuário",
    "Endereço de email",
    "Por favor, insira seu nome de usuário ou e -mail",
    "Senha",
    "Senha pelo menos 6 caracteres",
    "Confirme sua senha",
    "Código de afiliados (opcional)",
    "Verifique o código",
    "aceito",
    "INSCREVER-SE",
    "Não tem uma conta?",
    "Inscreva -se agora",
    "Seu código de verificação não pode estar vazio.",
    "Ao se inscrever, você concorda com o nosso",
    "política de Privacidade",
    "e",
    "Termos",
    "Criar uma conta",
    "Já tem uma conta?",
    "ENTRAR",
    "Faça login para continuar",
    "Seu email não pode estar vazio."
  ],
  "settle": [
    "Juros de liquidação",
    "A cada 24 horas"
  ],
  "product": [
    "Preço do contrato",
    "Insira o valor",
    "Período de investimento",
    "Lucro líquido diário",
    "Lucro líquido total",
    "Renda fixa",
    "Liquidar interesses",
    "A cada 24 horas",
    "Poder de hash",
    "Mineiro",
    "Bônus de afiliado",
    "Contrato de arrendamento de hash",
    "Comece agora",
    "Vendido",
    "Detalhes do contrato",
    "Dias",
    "Hash Power Marketplace",
    "Retornar após o término",
    "Dia",
    "Contratos de mineração",
    "Oferecemos uma variedade de contratos de investimento em mineração em nuvem, garantindo lucros estáveis ​​e confiáveis.",
    "DIA REWARDS FILL:",
    "Volume",
    "(Esgotado em 100%)",
    "Encomende agora",
    "Veja mais",
    "Confira",
    "Quantidade",
    "Senha",
    "Quantia",
    "Pagar"
  ],
  "password": [
    "Senha atual",
    "Nova Senha",
    "Confirme sua senha",
    "Confirmar alterações"
  ],
  "package": [
    "Txid | tempo",
    "Contrato",
    "Retornar",
    "Quantia",
    "Ganhos de mineração",
    "Data",
    "Do investimento",
    "Ganhos de mineração"
  ],
  "affliliate": [
    "Programa de afiliados",
    "INSCREVER-SE",
    "Junte -se à nossa comunidade, inscreva -se em uma conta, obtenha seu link de afiliados.",
    "Convide usuários",
    "Convide usuários, compartilhando seu link de afiliados via mídia social.",
    "Ganhe comissões",
    "Depois que um novo usuário se registrar e se inscrever no plano do contrato, você receberá uma recompensa de 3% do valor do plano de contrato.",
    "Junte -se ao nosso Programa de Afiliados (Programa Parceiro) e ganhe uma comissão vitalícia de até 3%! ",
    "Por exemplo, se alguém registrar uma conta usando seu código de afiliado e investe em um plano de US $ 100, você receberá US $ 3 gratuitamente. ",
    "Então, por que não aproveitar a oportunidade? ",
    "JUNTE-SE A NÓS"
  ],
  "affliliate2": [
    "Sobre afiliado",
    "Consulte um amigo e comece a ganhar com nosso programa de referência"
  ],
  "affliliate1": [
    "Nosso programa de afiliados oferece duas vantagens principais",
    "1. Oferecemos um programa de afiliados de topo que traz muitas vantagens. ",
    "2. Quando o número de usuários válidos que você recomenda no mês atingir o número necessário, você pode solicitar um bônus salarial mensal de promoção!",
    "① Recomende 8 novos usuários para se registrar e investir para atualizar para o VIP1 e acima, e você pode obter US $ 350 por mês",
    "② Recomende 25 novos usuários para se registrar e investir para atualizar para o VIP1 e acima, e você pode obter US $ 1.300 por mês",
    "③ Recomende 50 novos usuários para se registrar e investir para atualizar para o VIP1 e acima, e você pode obter US $ 4.000 por mês",
    "④ Recomende 100 novos usuários para se registrar e investir para atualizar para o VIP1 e acima, e você pode obter US $ 11.000 por mês",
    "⑤ Recomende 200 novos usuários para se registrar e investir para atualizar para o VIP1 e acima, e você pode obter US $ 22.000 por mês",
    "NOTA: Entre em contato com o atendimento ao cliente no último dia de cada mês para solicitar o salário de afiliado. ",
    "Junte-se a nós",
    "Junte -se ao nosso programa de parceiros e ganhe até 3%+1,5% de comissões ao longo da vida! ",
    "Por exemplo:",
    "A recomenda B para fazer investimentos adicionais; ",
    ""
  ],
  "clickmore": [
    "Clique em mais"
  ],
  "oursay": [
    "O que nossos clientes respeitáveis ​​dizem"
  ],
  "say": [
    "O DRML Miner é uma excelente plataforma de mineração em nuvem que pode ser usada para extrair criptomoedas para obter renda extra. ",
    "Eu trabalho nesta plataforma há quase um ano. ",
    "O DRML Miner é uma plataforma de mineração em nuvem muito boa. ",
    "Eu trabalho no DRML Miner há seis anos. ",
    "Sou muito grato ao DRML Miner por me ajudar quando só tinha US $ 10 no bolso há três anos. ",
    "A segurança da plataforma DRML Miner é muito boa. ",
    "Graças a Deus por me deixar conhecer o DRML Miner Esta plataforma não tem taxas ocultas para saques. ",
    "Eu sou uma mãe solteira. ",
    "Eu vi o artigo que eles compartilharam nas notícias. ",
    "Encontrei a plataforma DRML Miner na página de recomendação do Google. ",
    "Eu realmente gosto da confiabilidade geral da plataforma e do muitos suporte ao cliente, e as despesas diárias da plataforma são estáveis ​​e é muito fácil obter renda. ",
    "Fico feliz em compartilhar meus comentários sobre o DRML Miner novamente. ",
    "O efeito geral da plataforma DRML Miner é muito bom. ",
    "Esta é a minha primeira vez para entrar em contato com a mineração em nuvem como uma maneira de ganhar dinheiro. ",
    "O DRML Miner trata muito bem os recém -chegados. ",
    "Meu amigo apresentou o DDB para mim mineiro, recebi um bônus de dinheiro imediatamente após me inscrever. ",
    "Eu tenho usado o DRML Miner há um tempo. ",
    "Eu tentei muitas outras plataformas de mineração em nuvem. ",
    "Vou recomendar o DRML Miner a todos os entusiastas da criptomoeda. ",
    "O DRML Miner facilita a mineração de Bitcoin mais fácil do que nunca. ",
    ""
  ],
  "affiliates": [
    "Afiliados",
    "Programa de afiliados",
    "Seu e -mail",
    "Seu nome de usuário",
    "Seu código de referência",
    "Seu link de afiliado",
    "Ganhe ações de bônus",
    "Compartilhe com outros. Você receberá 3% mais como bônus de sua compra em cada pedido. ",
    "Saber mais",
    "Lista de parceiros",
    "Seus parceiros",
    "Data",
    "De",
    "Nível",
    "Quantia",
    "Tipo",
    "Link de cópia"
  ],
  "appDown": [
    "Comece a jornada de mineração",
    "Gerencie suas atividades {WT} em movimento usando nossos aplicativos móveis. ",
    "Gerencie sua receita a qualquer hora, em qualquer lugar",
    "Fácil começo com mineração",
    "Simplificamos todas as etapas necessárias para começar com a mineração em nuvem no aplicativo.",
    "Mineração com um clique",
    "Encomende sem esforço planos de mineração de longo e curto prazo com limites baixos e taxas baixas.",
    "Gerenciamento em tempo real",
    "Seja no local ou fora, monitore e gerencie sem esforço os ativos dos planos de mineração.",
    "Suporte de bate -papo 24/7",
    "Entre em contato com o suporte ao cliente com suas perguntas a qualquer momento."
  ],
  "contact": [
    "Entre em contato nos",
    "Nome",
    "E-mail",
    "Assunto",
    "Mensagem",
    "Digite Captcha",
    "Envie agora",
    "Bate -papo para apoiar",
    "Fale com nossa equipe amigável",
    "Converse para online",
    "Suporte ao vivo 24/7",
    "Entre em contato com o suporte ao cliente com suas perguntas a qualquer momento.",
    "Visite -nos",
    "Visite nossa sede no escritório"
  ],
  "dashboard": [
    "Ver tudo",
    "Saldo disponível",
    "Comissão Total",
    "Ordens ativas",
    "Ordens expiradas",
    "Pedidos totais",
    "Total de depósitos",
    "Total de retiradas",
    "Seu nome de usuário",
    "Seu código de referência",
    "Afiliados Link",
    "Link de cópia",
    "Parceiros",
    "Obtenha suporte agora",
    "Meu link de referência",
    "Fundos de teste",
    "Lucros totais",
    "Sistema"
  ],
  "faq": [
    "Perguntas frequentes"
  ],
  "home": [
    "Bem -vindo ao {WT}",
    "● O inscrição diária ganha $ 0,5",
    "Novos usuários podem receber US $ 10 quando se inscrever",
    "conhecer",
    "Como trabalhamos?",
    "Siga estas etapas simples e obtenha lucro!",
    "Registro instantâneo",
    "Clique",
    "Inscrever-se",
    "Para preencher o espaço em branco, nosso 256 SSL protegerá sua privacidade.",
    "Fazer um investimento",
    "Conecte-se",
    "Sua conta para clicar em Investir para começar a obter o lucro.",
    "obter lucro",
    "Você obterá seu lucro em seu perfil, também receberá pagamento instantâneo",
    "Grande visão. ",
    "Verde",
    "Energia",
    "Utilizamos painéis solares de alta qualidade para fonte de alimentação, garantindo lucros e protegendo o meio ambiente.",
    "Protegido por design",
    "A segurança dos dados e nossos servidores tem sido uma prioridade para nós do D-1. ",
    "Contratos lucrativos",
    "Encomende facilmente planos de mineração de longo e curto prazo com baixas barreiras e taxas baixas.",
    "Mercado estável",
    "Seus ganhos de mineração são resolvidos automaticamente na sua conta de plataforma a cada 24 horas.",
    "100% de segurança do fundo",
    "A maioria dos fundos é armazenada com segurança em carteiras frias offline. ",
    "Apoio amigável",
    "Nossa equipe de suporte está disponível 24 horas por dia, 7 dias por semana, para garantir que tudo esteja indo bem. ",
    "",
    "",
    "",
    "",
    "",
    ""
  ],
  "withdraw": [
    "Saldo disponível",
    "Rede de retirada",
    "Quantia",
    "Senha de pagamento",
    "A receber",
    "Enviar",
    "Txid",
    "Quantia",
    "Status de negociação",
    "Tempo",
    "Retirar",
    "Retirar registros",
    "Retirar endereço"
  ],
  "deposit": [
    "Depósito",
    "Criptomoeda",
    "Rede de depósito",
    "Registros de depósito",
    "Número de ID / hash de transação",
    "Por favor, preencha o ID / número de hash da transação",
    "Carregue a captura de tela de transferência",
    "Enviar",
    "Saldo disponível",
    "Método de depósito",
    "Quantia",
    "Limite",
    "A pagar",
    "Enviar",
    "Txid",
    "Método de depósito",
    "Quantia",
    "Status",
    "Tempo",
    "Detalhes",
    "Recompensa",
    "Depois da recompensa",
    "Taxa de conversão",
    "Valor a pagar",
    "Valor real",
    "Carregue a captura de tela de transferência",
    "Próximo",
    "Por favor, transfira para o seguinte endereço",
    "O sistema confirmará automaticamente o recibo na cadeia de blocos de memória e o publicará no saldo da sua conta.",
    "Valor mínimo do depósito:",
    ", quantidade menor que",
    "não será creditado em sua conta.",
    "Cada transferência será registrada na cadeia de blocos de memória e não pode ser adulterada com",
    "Endereço de transferência",
    "Converter",
    "Carregue a captura de tela",
    "Transfira o valor exato para o endereço"
  ],
  "limits": [
    "Cada conta é limitada a uma compra por dia.",
    "Cada usuário só pode comprar uma vez"
  ],
  "productDetail": [
    "Detalhe",
    "Serviços de taxa de hash e pós-venda fornecidos pelo DRML Miner.",
    "Os planos de taxa de hash de nuvem fornecem uma solução de mineração fácil e eficiente para clientes institucionais. \n",
    "Introdução ao produto\n",
    "1. Instruções de compra\n",
    "Antes de fazer um pedido, entenda o produto com cuidado. \n",
    "2. Liquidação de saída\n",
    "A renda estimada é derivada da operação em tempo real da rede de ativos digitais e está sujeita a alterações dinâmicas. \n",
    "3. Saída de cálculo\n",
    "Para o cálculo da receita de mineração, consulte o pool de minério de pool: https://www.poolin.com\n",
    "4.Ferminação do contrato\n",
    "O contrato será rescindido automaticamente quando expirar. Minina do contrato se tornará ativo imediatamente o pagamento de pagamento. A menos que seja fornecido de outra forma pela lei aplicável ou por uma oferta específica, todas as compras são finais e não reembolsáveis. Obrigado pelo seu entendimento.\n",
    "5. Dicas de risco\n",
    "O preço dos ativos digitais geralmente flutua bastante, e a dificuldade de mineração de cada moeda será ajustada regularmente. \n",
    "A plataforma se reserva o direito de interpretação final para os termos deste contrato.\n",
    "Isenção de responsabilidade e suporte\n",
    "1. Conforme especificado nos termos de uso do DRML Miner, uma vez que um pedido foi pago, é imutável e não reembolsável.\n",
    "2.DRML Miner will not be responsible for any results that arise from uncontrollable risks, including but not limited to natural disasters such as floods, volcanic eruptions, earthquakes, landslides, fires, storms, other unforeseen or higher risk weather events, government actions, or directives, city-level grid power supply incidents as well as social actions or conflicts such as war, strikes, riots, etc.\n",
    "3. A flutuação de hashrate é inevitável. \n",
    "4. A produção estática foi calculada estaticamente pela dificuldade atual, que flutua com o mercado. ",
    "A plataforma se reserva o direito de interpretação final para os termos deste contrato.\n",

  ],
  "blog": [
    "Blog recente",
    "Postagens recentes do blog",
    "Leia mais"
  ],
  "welfare": [
    "Clube VIP"
  ],
  "tips": [
    "Após o pagamento bem -sucedido, salve a captura de tela de pagamento no seu telefone celular e faça o upload da captura de tela para a página antes de enviar o pedido. ",
    "A captura de tela deve conter o hash da transação, caso contrário, o sistema não pode reconhecê -lo."
  ],
  "next": "Próximo pagamento",
  "placehoder": [
    "Por favor, insira o valor",
    "Por favor, insira senha de pagamento",
    "A senha de login e a senha de pagamento são recomendadas para serem diferentes, lembre -se da senha."
  ],
  "dep": [
    "Lembrete quente: a quantidade mínima de recarga começa em US $ 100. ",
    "Dicas: o valor mínimo de retirada começa a partir de US $ 100. Se você não sabe como retirar ou não possui a senha, entre em contato com o atendimento ao cliente on -line para obter ajuda."
  ],
  "deposit1": [
    "Você solicitou",
    "Por favor, pague",
    "para pagamento bem -sucedido",
    "Depois que o pagamento for bem -sucedido, salve a captura de tela e faça o upload da captura de tela conforme solicitado."
  ],
  "header": [
    "LAR",
    "PAINEL",
    "Minhas contas",
    "DEPÓSITO",
    "RETIRAR",
    "Minhas ordens",
    "Contratos",
    "Programa de afiliados",
    "CONFIGURAÇÕES",
    "SOBRE NÓS",
    "Centro de ajuda",
    "Configurações",
    "Minhas contas",
    "ENTRAR",
    "INSCREVER-SE",
    "SAIR"
  ],
  "home1": [
    "Fácil mineração em nuvem a qualquer hora, em qualquer lugar",
    "",
    "",
    "O DRML Miner é a principal plataforma de mineração em nuvem do mundo, usando energia limpa combinada com equipamentos de nível superior para fornecer aos usuários uma maneira simples e conveniente de minerar. ",
    "Comece seu investimento em mineração em nuvem, retire seu primeiro pagamento após 24 horas.",
    "Comece a minerar",
    "Como começar",
    "INSCREVER-SE",
    "Preencha o formulário e prepare -se para começar a minerar em 1 minuto.",
    "Escolha seus contratos",
    "Fornecemos contratos de mineração de nuvem lucrativos e sem riscos.",
    "Comece a ganhar",
    "Após a compra do contrato, os lucros diários serão prontamente depositados em sua conta.",
    "Escolha contratos de mineração",
    "Oferecemos uma variedade de contratos de investimento em mineração em nuvem, garantindo lucros estáveis ​​e confiáveis.",
    "Por que nos escolher?",
    "Lucros estáveis",
    "Oferecemos renda fixa constante que é depositada automaticamente em sua conta todos os dias.",
    "Pagamentos regulares rápidos",
    "Todas as solicitações de retirada serão processadas automaticamente pelo sistema em cinco minutos.",
    "Inclusão",
    "Nossa experiente equipe de investimento e especialistas em TI fornecerão a melhor tecnologia líder de mercado.",
    "Equipe especialista",
    "Nossa plataforma oferece suporte ao investimento em mineração em nuvem para todos, mesmo que você seja iniciante, pode começar rapidamente e se beneficiar com ele.",
    "Nossos parceiros",
    "Aceite uma variedade de métodos de pagamento",
    "Para garantir a conveniência de nossos investidores, {WT} fornece uma gama diversificada de métodos de pagamento para depositar fundos e receber pagamentos.",
    "Investimento de mineração em nuvem no Texas",
    "<p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Bem -vindo ao {WT}, seu ingresso para o emocionante mundo da mineração de criptomoedas.</p><p class=\"mt-2 s-content\" ><span class=\"bullet-icon\" >•</span>No coração do Texas, a jornada de {WT} começa, alavancando as vastas recursos e proezas tecnológicas do estado para fornecer soluções de mineração de nuvem de ponta.</p><p class=\"mt-2 s-content\" ><span class=\"bullet-icon\" >•</span>Em {WT}, não somos apenas sobre mineração; somos sobre inovação. </p><p class=\"mt-2 s-content\" ><span class=\"bullet-icon\" >•</span>Nosso hardware de mineração em nuvem em Washington possui equipamentos de primeira linha, garantindo o desempenho e a confiabilidade ideais. </p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Diga adeus a configurações complexas; com o Bitcoin de mineração em nuvem na Califórnia, você pode acessar nossos serviços on-line. Nosso serviço de mineração de bitcoin foi projetado com facilidade de uso em mente, atendendo a iniciantes e especialistas.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Levamos sua segurança a sério, oferecendo serviços confiáveis ​​de mineração em nuvem apoiados por medidas robustas. {WT} é conhecido por sua transparência e confiabilidade, tornando -nos uma escolha preferida para a mineração em nuvem, não apenas no Arizona, mas nos EUA.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Nossa dedicação à tecnologia blockchain é evidente em nossa mineração de nuvem de blockchain nos EUA. Utilizamos blockchains públicos para experiências aprimoradas de mineração e nossa segurança aprimorada de mineração em nuvem garante a segurança de seus ativos.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>De Litecoin ao Ethereum, temos você coberto. </p><p class=\"mt-2 s-content\" >Mas não termina aí. {WT} oferece uma oportunidade única para você lucrar com sua paixão. Junte nossos programas de afiliados de mineração em nuvem e programa de afiliados criptográficos nos EUA, transformando seu interesse pela criptomoeda em uma fonte de renda.</p><p class=\"mt-2 s-content\"><span class=\"bullet-icon\">•</span>Com {WT}, sua jornada para o universo de criptomoeda começa.</p>",
    "29"
  ],
  "latest": [
    "Últimas transações",
    "Mais recente recarga",
    "Últimas retiradas",
    "Usuário",
    "Quantia",
    "Tempo"
  ],
  "reg": [
    "Por favor, digite seu e -mail"
  ],
  "tutorial": [
    "Tutorial"
  ],
  "bounty": [
    "Recompensa"
  ],
  "invest": [
    "Invista agora"
  ],
  "ppp": [
    "Este contrato não participa de recompensas da aliança",
    "(Este contrato pode ser ilimitado ou várias cópias podem ser adquiridas ao mesmo tempo)"
  ],
  "buy": [
    "Comprar agora",
    "Insira fundos de compra"
  ],
  "aa": [
    "Copie o endereço da carteira da plataforma e transfira os fundos por meio da retirada da troca de \"sua\". "
  ],
  "tag": [
    "Por favor, preencha a tag XRP:"
  ],
  "about1": [
    "SOBRE NÓS",
    "O DRML Miner é a principal plataforma de mineração em nuvem do mundo, usando energia limpa combinada com equipamentos de nível superior para fornecer aos usuários uma maneira simples e conveniente de minerar. ",
    "O DRML Miner foi fundado em 2018 e está sediado no Reino Unido. ",
    "Curso da empresa: melhoria contínua e inovação",
    "Após anos de desenvolvimento e crescimento contínuo, atualmente possui mais de 100 minas de energia ambientalmente amigável em larga escala em todo o mundo, com usuários em 180 países e regiões. ",
    "Missão: liderando uma nova era de energia verde",
    "Estamos comprometidos em nos tornar um líder inovador no campo da computação em nuvem e mudar a maneira como os humanos usam energia. ",
    "Visão: Construir uma marca líder global na indústria de computação em nuvem",
    "Nosso objetivo é se tornar um líder global em tecnologia inovadora de mineração em nuvem verde, fornecer produtos e serviços de valor agregado aos usuários e criar um excelente valor de investimento e altos retornos. ",
    "Capacite seus ganhos com o DRML Miner",
    "Experimente a mineração digital perfeita, segura e lucrativa com a tecnologia e os serviços de ponta da DRML Miner.",
    "O meu mais inteligente, ganha mais rápido - o DRML Miner facilita!",
    "Sem hardware, sem problemas - apenas recompensas puras de bitcoin.",
    "Maximize os lucros com as soluções de mineração inteligentes da DRML Miner.",
    "Seguro, escalável e sem costura - o futuro da mineração de criptografia.",
    "Sua jornada de mineração começa aqui - junte -se a Drml Miner hoje!"
  ],
  "download1": [
    "Faça o download do aplicativo móvel para começar a minerar hoje!",
    "Clique para baixar diretamente o aplicativo DRML Miner Mobile para gerenciar suas atividades de mineração a qualquer hora, em qualquer lugar, desfrutar de uma experiência mais rápida e ajudá -lo a aumentar sua renda com eficiência."
  ],
  "newhome": [
    "Aproveitando a energia verde para impulsionar um futuro sustentável para a mineração em nuvem",
    "O Data Data Center DRML Miner usa energia verde limpa e de baixo custo (principalmente hidrelétrica, energia eólica, energia solar, etc.), pratica conceitos de proteção ambiental, promove a inovação em economia de energia e está comprometida em construir um data center verde, sustentável e que economiza energia.",
    "Registrar uma conta",
    "Clique na opção Página da Web (inscreva -se) para preencher o formulário",
    "Selecione contrato",
    "Escolha seu contrato de mineração",
    "Obter lucro",
    "A receita estável do contrato é atualizada automaticamente para sua conta",
    "Por que nos escolher?",
    "Vantagens do mineiro drml",
    "Hardware mais recente",
    "Utilizamos as mais recentes máquinas de mineração ASIC e equipamentos de GPU e utilizamos energia limpa combinada com equipamentos de nível superior para fornecer aos usuários uma maneira simples e conveniente de minerar.",
    "Sem taxas adicionais",
    "Não é necessário hardware, fornecemos o poder de computação e a plataforma cuida do processo de mineração, você não precisa pagar as taxas de instalação ou custos adicionais de manutenção e eletricidade. ",
    "Ganhos estáveis",
    "Depois de comprar seu contrato, nosso sistema começa automaticamente a funcionar e processa pagamentos a cada 24 horas.",
    "Financia a segurança",
    "A maioria dos fundos é armazenada com segurança em carteiras frias offline. ",
    "Equipe especialista",
    "Nossa equipe de mineração consiste em profissionais da indústria blockchain e engenheiros de TI, de pé 24/7 para garantir que tudo corra bem, e estamos sempre aqui para ajudar.",
    "Mineração ecológica",
    "Nossas máquinas de mineração e sistemas de refrigeração são alimentadas pelos melhores painéis solares monocristalinos e também utilizam energia eólica em larga escala para garantir lucros e proteger melhor o meio ambiente.",
    "O que você obterá como parceiro afiliado?",
    "Participe do nosso programa de referência (Programa Parceiro) e ganhe até 1,5% -4,5% da Comissão para a Vida! ",
    "Primeiro, inscreva -se em uma conta e obtenha seu próprio link de referência/URL.",
    "Em seguida, convide novos usuários compartilhando seu link de referência através da mídia social ou do seu próprio site. ",
    "Quando um novo usuário se registra usando seu link de referência, você receberá um bônus de 1,5% -4,5% do valor da compra. ",
    ""
  ],
  "complete": "concluído",
  "cancel": "Cancelar",
  "confirm": "Confirmar"
}
